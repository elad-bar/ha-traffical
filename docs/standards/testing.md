...
**HA plugin skip list** (`_HASS_TEST_FILES`): `test_config_flow.py`, `test_coordinator.py`, `test_setup_unload.py`, `test_device_trigger.py`. Changelog and HA-free tests run on Windows and Linux.

All other tests still run on a Windows host. A green native Windows `pytest` is **not** the same as CI if you changed config flow, coordinator, setup/unload, or device triggers.
...
| Config flow, coordinator, load/unload, device triggers | `test_config_flow.py`, `test_coordinator.py`, `test_setup_unload.py`, `test_device_trigger.py` (full suite: CI / WSL / Linux container) |
