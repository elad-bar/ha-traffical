...
def _morning(service_date: str, **kwargs):
    row = {
        ...
        "rideInfo": {
            ...
            "endDateTime": f"{service_date}T07:12:00",
    ...
    details = {
        ...
        "endTime": f"{service_date}T07:12:00",
        "status": "New",
        "stations": [
            {
                "name": "Home",
...
def test_item_from_occurrence_includes_actuals() -> None:
    item = item_from_occurrence(_morning("2026-09-02"))
    assert item is not None
    assert item.uid == "392681:120:2026-09-02"
    assert item.summary == "Morning line"
    assert "actual 07:15" in item.description
    assert "Home" in item.location
